...
function get_runtimes_3d(cell; Ns = [2, 20, 200], Ts = [1, 8, 16, 64])
    ...
    Random.seed!(10)
    @showprogress 1 "MatrixSize: " for n in Ns
        @showprogress 1 "TemporalLength: " 1 for t in Ts
            x = randn(Float32, n, n, t)
            rnn_model = cell(n, n)

            timings = run_benchmark_recur_3d(rnn_model, x, cuda=false)
            for k in keys(timings)
                push!(RUNTIMES[k], ("$(cell)", false, n, t, get_statistics(timings[k])...))
            end
...
function run_benchmark(cells=[RNN, GRU, LSTM]; save_dir="results", kwargs...)
